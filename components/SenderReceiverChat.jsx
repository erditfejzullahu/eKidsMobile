                    ...
                    <View className="relative">
                        {renderItem?.content && renderItem?.fileUrl ? (
                            ...
                            <View className="bg-oBlack border border-black-200 p-4 rounded-[5px] mb-4 mt-2" style={styles.box}>
                                ...
                                <Text style={styles.box} className="text-white font-psemibold text-xs absolute bottom-0 left-0 px-2 py-0.5 rounded-bl-[5px] rounded-tr-[5px] bg-secondary border-t border-r border-black-200">{renderItem?.instructor && "Qe nga"} {renderItem?.onlineMeeting === null ? formateSharedItemDates : (renderItem?.onlineMeeting?.status === 0 || renderItem?.onlineMeeting?.status === 3) ? <CountdownTimer meetingData={renderItem?.onlineMeeting}/> : "Ka perfunduar"}</Text>
