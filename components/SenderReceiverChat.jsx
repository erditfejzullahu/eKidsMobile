...
const SenderReceiverChat = ({renderItem, currentUser, conversationUserData}) => {
  ...
  return (
    <>
    <View className="px-4" style={shadowStyle}>
        {renderItem?.senderUsername === currentUser?.username ? (
            <View className={`flex-row-reverse items-center gap-4`}>
                <View className="bg-gray-200 dark:bg-oBlack mb-auto rounded-full border border-white dark:border-black-200 p-2">
                    <Image 
                ...
                <View className={`p-4 rounded-[10px] flex-1 border border-gray-200 dark:border-black-200 bg-primary-light dark:bg-primary`}>
                    <View>
                        <Text className="font-psemibold text-lg text-oBlack dark:text-white">{currentUser?.firstname} {currentUser?.lastname}</Text>
                    </View>
                    <View className="relative">
                        {renderItem?.content && renderItem?.fileUrl ? (
                            <>
                                ...
                                <View>
                                    <Text className="font-plight text-sm text-gray-600 dark:text-gray-400 mb-2">{renderItem?.content}</Text>
                                </View>
                            </>
                        ) : renderItem?.content && !renderItem?.fileUrl ? (
                            <Text className="font-plight text-sm text-gray-600 dark:text-gray-400 mb-2">{renderItem?.content}</Text>
                            ...
                            <View className="bg-oBlack-light dark:bg-oBlack border border-gray-200 dark:border-black-200 p-4 rounded-[5px] mb-4 mt-2" style={shadowStyle}>
                                ...
                                <Text style={shadowStyle} className="text-white font-psemibold text-xs absolute bottom-0 left-0 px-2 py-0.5 rounded-bl-[5px] rounded-tr-[5px] bg-secondary border-t border-r border-white dark:border-black-200">{renderItem?.instructor && "Qe nga"} {renderItem?.onlineMeeting === null ? formateSharedItemDates : (renderItem?.onlineMeeting?.status === 0 || renderItem?.onlineMeeting?.status === 3) ? <CountdownTimer meetingData={renderItem?.onlineMeeting}/> : "Ka perfunduar"}</Text>
                                {(renderItem?.course || renderItem?.lesson || renderItem?.quiz || renderItem?.blog?.categoryId || renderItem?.instructorLesson?.categoryId || renderItem?.instructorCourse?.categoryId) ? (
                                    <Text style={shadowStyle} className="text-oBlack dark:text-white font-psemibold text-xs absolute bottom-0 right-0 px-2 py-0.5 rounded-br-[5px] rounded-tl-[5px] bg-gray-200 dark:bg-oBlack border-t border-l border-white dark:border-black-200">{getCourseCategories(categories, renderItem?.course ? renderItem?.course?.courseCategory : renderItem?.lesson ? renderItem?.lesson?.courseCategory : renderItem?.quiz ? renderItem?.quiz?.quizCategory : renderItem?.blog ? renderItem?.blog?.categoryId : renderItem?.instructorLesson ? renderItem?.instructorLesson?.categoryId : renderItem?.instructorCourse ? renderItem?.instructorCourse?.categoryId : 1)}</Text>
                                ) : (
                                    <Text style={shadowStyle} className="text-oBlack dark:text-white font-psemibold text-xs absolute bottom-0 right-0 px-2 py-0.5 rounded-br-[5px] rounded-tl-[5px] bg-gray-200 dark:bg-oBlack border-t border-l border-white dark:border-black-200">{renderItem?.blog ? "Blog" : renderItem?.instructorLesson ? "Leksion" : renderItem?.instructorCourse ? "Kurs" : renderItem?.instructor ? "Instruktor" : renderItem?.discussion ? "Diskutim" : renderItem?.onlineMeeting ? "Takim Online" : "None"}</Text>
                                )}
                            </View>
                            </>
                        ) : (<Text className="text-oBlack dark:text-white text-sm italic">Mesazhi eshte fshire</Text>)}
            ...
            <View className={`flex-row items-center gap-4`}>
                <View className="bg-oBlack-light dark:bg-oBlack rounded-full border border-gray-200 dark:border-black-200 p-2">
                    <Image 
                ...
                <View className={`p-4 rounded-[10px] flex-1 border bg-gray-200 dark:bg-oBlack border-white dark:border-black-200`}>
                    <View>
                        <Text className="font-psemibold text-lg text-oBlack dark:text-white">{conversationUserData?.receiverFirstname} {conversationUserData?.receiverLastname}</Text>
                    </View>
                    <View className="relative">
                        {renderItem?.content && renderItem?.fileUrl ? (
                            <>
                            ...
                            <View className="bg-primary-light dark:bg-primary border border-white dark:border-black-200 p-4 rounded-[5px] mb-4 mt-2" style={shadowStyle}>
                                <TouchableOpacity
                                    onPress={() => handleGoToLocation(renderItem)}
                                    className="absolute top-0 right-0"
                                >
                                    <Animatable.Text animation="pulse" iterationCount="infinite" duration={1000}
                                        className="text-white font-psemibold text-xs absolute right-0 px-2 py-0.5 rounded-bl-[5px] rounded-tr-[5px] bg-secondary border-b border-l border-gray-200 dark:border-black-200"
                                        style={shadowStyle}
                                ...
                                <Text style={shadowStyle} className="text-white font-psemibold text-xs absolute bottom-0 left-0 px-2 py-0.5 rounded-bl-[5px] rounded-tr-[5px] bg-secondary border-t border-r border-gray-200 dark:border-black-200">{renderItem?.instructor && "Qe nga"} {renderItem?.onlineMeeting === null ? formateSharedItemDates : (renderItem?.onlineMeeting?.status === 0 || renderItem?.onlineMeeting?.status === 3) ? <CountdownTimer meetingData={renderItem?.onlineMeeting}/> : "Ka perfunduar"}</Text>
                                {(renderItem?.course || renderItem?.lesson || renderItem?.quiz || renderItem?.blog?.categoryId || renderItem?.instructorLesson?.categoryId || renderItem?.instructorCourse?.categoryId) ? (
                                    <Text style={shadowStyle} className="text-oBlack dark:text-white font-psemibold text-xs absolute bottom-0 right-0 px-2 py-0.5 rounded-br-[5px] rounded-tl-[5px] bg-gray-200 dark:bg-oBlack border-t border-l border-white dark:border-black-200">{getCourseCategories(categories, renderItem?.course ? renderItem?.course?.courseCategory : renderItem?.lesson ? renderItem?.lesson?.courseCategory : renderItem?.quiz ? renderItem?.quiz?.quizCategory : renderItem?.blog ? renderItem?.blog?.categoryId : renderItem?.instructorLesson ? renderItem?.instructorLesson?.categoryId : renderItem?.instructorCourse ? renderItem?.instructorCourse?.categoryId : 1)}</Text>
                                ) : (
                                    <Text style={shadowStyle} className="text-oBlack dark:text-white font-psemibold text-xs absolute bottom-0 right-0 px-2 py-0.5 rounded-br-[5px] rounded-tl-[5px] bg-gray-200 dark:bg-oBlack border-t border-l border-white dark:border-black-200">{renderItem?.blog ? "Blog" : renderItem?.instructorLesson ? "Leksion" : renderItem?.instructorCourse ? "Kurs" : renderItem?.instructor ? "Instruktor" : renderItem?.discussion ? "Diskutim" : renderItem?.onlineMeeting ? "Takim Online" : "None"}</Text>
    ...
    >
        ...
        >
            <View className="flex-1 justify-center items-center" style={{backgroundColor: "rgba(0,0,0,0.4)"}}>
                ...
                <TouchableWithoutFeedback>
                    <View style={shadowStyle} className="bg-primary-light dark:bg-primary p-4 rounded-[5px] border border-gray-200 dark:border-black-200 w-[200px] items-center">
                        <View>
                            <Text className="font-pbold text-oBlack dark:text-white text-sm">E derguar me:</Text>
                            <Text className="font-plight text-gray-600 dark:text-gray-400 text-sm">{formattedDateExtended}</Text>
                        </View>
                        ...
                        <View className="absolute -top-1 -right-1">
                            <TouchableOpacity onPress={() => setModalVisible(false)} className="bg-oBlack dark:bg-white p-1 rounded-full">
                                <Image 
                                    source={icons.close}
