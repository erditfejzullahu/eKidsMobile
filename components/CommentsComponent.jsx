...
const CommentsComponent = ({commentData, handleLessonLike, handleReplyComment, handleCommentLike }) => {
                ...
                <View key={item?.commentId} className="flex-1 flex-col w-full">
                    <View className={`flex-row gap-2 p-3 flex-1 ${commentIndex === 1 ? "ml-8" : "ml-16"}`}>
                        <View className="flex-[0.2] items-center relative">
                            <View className="border border-black-200 bg-oBlack rounded-[10px] h-14 w-14 items-center justify-center relative">
                                <Image
                                    source={icons.profile}
                                    resizeMode="contain"
                                    className="h-8 w-8"
                                />
                                <View className="absolute w-full h-2 -bottom-2 bg-primary -z-10" />
                            </View>
                            {showRepliesComments && <View className=" absolute w-[1px] right-4 -z-20 h-full bg-black-200 -bottom-4" />}
                        </View>
                        <View className="flex-1 flex-col gap-1">
                            ...
                            <View className="flex-row gap-4 items-center">
                                ...
                                >
                                    <Text className={`${item?.isLiked ? "text-secondary" : "text-white"} text-xs font-pbold`}>{item?.isLiked ? "I Pelqyer" : "Pelqeni"}</Text>
                                    <View className="flex-row items-center">
                                        ...
                                        <View className="-ml-3.5">
                                            <Text className="font-psemibold text-sm text-white">{item?.likes}</Text>
                                        </View>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => makeReply(item?.commentId)}>
                                    <Text className="text-white text-xs font-pbold">Repliko</Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            {(showReply.includes(item?.commentId) &&
                                <View className="flex-1 border rounded-[10px] border-black-200 p-2 bg-oBlack">
                                    <View>
                                        <Text className="text-gray-400 font-plight text-xs">Duke replikuar ne komentin e <Text className="text-secondary font-psemibold">{interactions?.user?.name}</Text></Text>
                                    </View>
                                    <View className="flex-row gap-2 w-full mt-1.5">
                                        <View className="flex-[0.2] items-center justify-center border border-black-200 rounded-[10px]">
                                            <Image
                                                source={icons.profile}
                                                resizeMode='contain'
                                                className="h-8 w-6"
                                        ...
                                        <View className="flex-1 ">
                                            ...
                                            <TextInput
                                                className="text-white font-pregular flex-1 border border-black-200 p-4 rounded-[10px]"
                                                value={commentValue}
    ...
    return (
        <>
            <View  className="relative flex-col mb-4 justify-between flex-1 border-l border-r border-black-200 w-full">
                <View className="gap-2 w-full flex-1 border-b border-black-200 pb-4">
                    <Animatable.View animation="fadeInLeft" duration={700} key={interactions?.commentId} className={`flex-row gap-2 p-2`}>
                            <View className="flex-[0.2] items-center relative">
                                ...
                                <View className="absolute w-[1px] right-4 -z-20 h-full bg-black-200 -bottom-4" />
                            </View>
                            <View className="flex-[1] flex-col gap-1">
                                <View>
                                    <Text className="text-gray-400 font-psemibold text-xs">{interactions?.user?.name}</Text>
                                    <Text className="text-white font-plight text-sm">{interactions?.comment_Content}</Text>
                                </View>
                                <View className="flex-row gap-4 items-center">
                                    <TouchableOpacity onPress={() => prepareLikeComment(interactions)} className="flex-row gap-1 items-center justify-center">
                                        <Text className={`${interactions?.isLiked ? "text-secondary" : "text-white"} text-xs font-pbold`}>{interactions?.isLiked ? "I Pelqyer" : "Pelqeni"}</Text>
                                        <View className="flex-row items-center">
                                            <Image 
                                                source={icons.star}
                                                resizeMode='contain'
                                            ...
                                            />
                                            <View className="-ml-3.5">
                                                <Text className="font-psemibold text-sm text-white">{interactions?.likes}</Text>
                                            </View>
                                        </View>
                                    </TouchableOpacity>
