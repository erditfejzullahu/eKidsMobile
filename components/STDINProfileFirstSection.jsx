...
import { getCourseCategories, InstructorCreatedCoursesById } from '../services/fetchingService'
import EmptyState from './EmptyState'

const STDINProfileFirstSection = ({data, userData}) => {
    const [courseModal, setCourseModal] = useState(true)
  ...
  return (
    ...
    <View className="relative items-center justify-center mt-20">
        ...
        {data?.isYourInstructor ? (
            ...
            <TouchableOpacity className="py-1 px-3 border border-white bg-secondary rounded-md mt-2" style={styles.box}>
    ...
    >
        {coursesData.length > 0 && <Text className="text-gray-400 font-plight text-xs mt-1 text-center">Me ane te listes se gjeneruar me poshte, ju mund te zgjidhni nje kurs ne te cilin mund te beheni studente i <Text className="text-secondary">{data?.instructorName}</Text></Text>}
        {coursesData.length !== 0 ? (
            ...
            {coursesData.map((item) => {
                ...
                return (
                    <TouchableOpacity key={item.id} className="bg-oBlack p-2 border-b relative border-black-200 rounded-b-md" style={styles.box}>
